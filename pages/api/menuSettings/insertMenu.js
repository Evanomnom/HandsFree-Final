...
handler.post(async (req, res) => {
    ...
    let docTwo = await req.db.collection(data.url).findOne({menuCheck:true, name:{$exists:true}})
    if (docTwo) {
        await req.db.collection(data.url).updateOne({menuCheck: true},{$set: {Menu : data.menu, menuCheck: true}}, {upsert: true})
    } else {
        await req.db.collection(data.url).updateOne({menuCheck: true},{$set: {Menu : data.menu, name: data.name, menuCheck: true}}, {upsert: true})
    }
    res.send({message: "ok"});
});
